...
- 赛后成绩结算：队长提交队名、到点顺序和总用时，系统按线路校验点位是否齐全、顺序是否一致；漏点或顺序错误也保留比赛记录并在成绩栏注明原因。同一队伍重复提交只保留先完成的一份；裁判可在总览查看各队耗时与问题，对录错成绩撤销一次，撤销后队伍回到待结算，原成绩不再参与排名。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/routes` | 线路列表及规定点位顺序 |
| POST | `/api/results/submit` | 队长提交队名、到点顺序（`submittedSequence`）、总用时秒数（`totalSeconds`） |
| GET | `/api/overview` | 裁判总览：排名、各队耗时、未完成原因、待结算队伍、撤销历史（可加 `?route=<id>`） |
| POST | `/api/results/<id>/revoke` | 裁判撤销一次录错成绩，撤销后队伍回到待结算 |

结算规则：

- 点位齐全且顺序一致为完成，按总用时升序排名；漏点或顺序错误保留记录，成绩栏注明原因且不参与排名。
- 同一队伍同一线路只保留一份有效成绩，重复提交返回 `409`；撤销后可重新提交，每条记录只能撤销一次（重复撤销返回 `400`）。

后端单元测试：

```bash
cd backend
python manage.py test domain
```
