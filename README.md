...
- 赛后成绩结算：队长提交队名、到点顺序与总用时，系统按线路校验点位是否齐全、顺序是否一致；漏点或顺序错误也保留比赛记录并在成绩栏注明未完成原因（未完赛不参与排名）；同一队伍重复提交只保留先完成的一份；裁判可在总览查看各队耗时与问题，录错时可撤销一次，撤销后队伍回到待结算、原成绩留痕且不再参与排名。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/course` | 获取当前生效线路、规定到点顺序与参赛队伍名单 |
| POST | `/api/results/submit` | 队长提交成绩：`{teamName, sequence, totalTime}`，totalTime 支持 `mm:ss`/`hh:mm:ss` 或秒数 |
| GET | `/api/results/overview` | 裁判总览：统计、各队成绩、实时排名、撤销留痕 |
| POST | `/api/results/{id}/revoke` | 裁判撤销一次录错成绩：`{reason?}`，队伍回到待结算 |

业务规则：

- `finished`（点位齐全且顺序一致）按总用时升序排名；`incomplete`（漏点/顺序错误/线路外点）保留记录但不进排名。
- 同一队伍已存在已结算成绩时，再次提交返回 `409 duplicate_submission`，只保留先完成的一份。
- 每条成绩最多撤销一次：原成绩标记为 `revoked` 留痕，同时为该队重新生成 `pending` 记录；重新提交后产生的新成绩仍可再撤销一次。
